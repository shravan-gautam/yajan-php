...
class alter_system
{
	...
	{
		global $libVersion,$CS,$FREAMWORK_PATH,$YAJAN_SERVER,$_CONSOLE;		
		$info = new YajanInfo();
		if($cmd[3]=="update")
		{
			...
			$avReview= file_get_contents($YAJAN_SERVER."/release.php?version=$libVersion&get=updateVersion");
			...
			for($i=0;$i<count($data);$i++)
			{
				...
				if($filename!="")
				{
					...
					if(file_exists("$filename"))
					{
						...
						{
							$filedata = file_get_contents($YAJAN_SERVER."/release.php?hash=$mdHash");
			...
			$data = file_get_contents($YAJAN_SERVER."/release.php?version=$libVersion&get=patchlist&cv=$cv");
			$data = explode("\n",$data);
			for($i=0;$i<count($data);$i++)
			{
				$data[$i] = str_replace("\n","",$data[$i]);
