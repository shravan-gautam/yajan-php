...
class Table extends UIObject
{
	...
	function addDefaultCss($key,$val)
	{
		for($i=0;$i<count($this->columns);$i++)
		{
	...
	function setDefaultValueFormat($format)
	{
		for($i=0;$i<count($this->columns);$i++)
		{	
		...
		if(!$this->recordset->isPopulated())
		{
			...
			while($row = $this->recordset->getDBRow())
			{
				...
				if($this->showRownum)
				{
					...
					if($this->recordsetOverwrite==true)
						{
							$stObj->setValue($i+1);
						}
						else
						{
							$stObj->setValue($i+1+$from);
						}
					$stObj->setEcho(false);
