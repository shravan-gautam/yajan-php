...
class Recordset
{
	...
	function setPriviousColumnReset($v)
	{
		$this->setPriviusColumnReset($v);
	}
			...
			for($i=0;$i<$t->count;$i++)
			{
				if(isset($t->data[$i][$col]))
				{
					if($mode=="==")
					{
						...
						{
							if(array_search(trim($t->data[$i][$col]),trim($val))!==false)
							{
	...
	function removeColumns()
	{
		...
		for($i=0;$i<count($this->columns);$i++)
		{
			...
			foreach ($arg_list as $key => $value) {
				...
				if($this->columns[$i]->getName()==$value)
				{
					$exist = true;
				}
			}
		...
		for($i=0;$i<$this->count();$i++)
		{
			...
			{
				# code...
				if(array_search($key,$arg_list)===false)
				{
					$o[$key]=$value;
				}
			}
	...
	function getMultiLevelDataset()
	{
		$b = new MultiLevelDataset();
		for($i=0;$i<$this->count;$i++)
		{
			$b->add($this->data[$i]);
		}
	...
	function toFile($filename="")
	{
		...
		if($filename=="")
		{
			$this->message="destination filename is null";
			return false;
		}
		$file = new File($filename);
	...
	function fromFile($filename)
	{
		...
		$file = new File($filename);
		return $this->fromString($file->read());
	}
	...
	function fromArray($arr)
	{
		...
		{
			$v = array_change_key_case($arr[$i],CASE_UPPER);
			$this->add($v);
		}
	...
	function get($name,$i=-1)
	{
		...
		//echo "A";
		return $this->data[$this->recordPosition][strtoupper($name)];
	}
	function set($name,$value,$i=0)
	{
		if($i==0 && $this->recordPosition!=0)
		{
			$i=$this->recordPosition;
		}
		$this->data[$i][strtoupper($name)]=$value;
	}
	function count()
	{
			return $this->count;	
