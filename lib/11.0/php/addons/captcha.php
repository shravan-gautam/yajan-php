...
class Captcha
{
	...
	{
		$this->id= $id;
		$this->obj = new CoreCaptcha();
		$this->inputRander = false;
		$this->obj->session_var=$id."_capchaVal";
	}
	function showForm()
	{
		$text = new TextBox($this->id."_input");
		$text->rander();
		$this->inputRander=true;
	}
	function rander()
	{
		$this->obj->CreateImage();
		if($this->inputRander==false)
		{
			$this->showForm();
		}
	}
	function isValid()
	{
		if (!empty($_REQUEST[$this->id."_input"])) 
		{
			if (empty($_SESSION[$this->obj->session_var]) || trim(strtolower($_REQUEST[$this->id."_input"])) != $_SESSION[$this->obj->session_var]) 
			{
				return false;
			} 
			else 
...
class CoreCaptcha {
    ...
    public function __construct($config = array()) {
		global $_PWD,$FREAMWORK_PATH,$libVersion;
		$this->resourcesPath = "$_PWD/$FREAMWORK_PATH/lib/$libVersion/php/addons/captcha";
    }
    ...
    protected function WriteImage() {
		...
		ob_end_clean();
		$data = base64_encode($resizedJpegData);
		echo '<img width="200px" height="100px;" src="'."data:image/jpeg;base64,".$data."\" />";
    }
