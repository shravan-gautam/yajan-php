...
class TPEpubCreator
{
    ...
    public function CreateEPUB() {
        ...
        $indexPage = "<?xml version='1.0' encoding='utf-8'?>".'
        ...
        <body><h3>अनुक्रमणिका</h3><ul class="indexList">';

        $opf .= '<item id="page0" href="page0.xhtml" media-type="application/xhtml+xml" />' . "\r\n";
        $ncx  .= '<navPoint id="page0" playOrder="0">' . "\r\n";
        $ncx .= '<navLabel>' . "\r\n";
        $ncx .= '<text>अनुक्रमणिका</text>' . "\r\n";
        $ncx .= '</navLabel>' . "\r\n";
        $ncx .= '<content src="page0.xhtml"/>' . "\r\n";
        $ncx .= '</navPoint>' . "\r\n";
        $fill_opf_spine .= '<itemref idref="page0" />' . "\r\n";
        ...
        foreach( $this->pages as $key => $value ) {
            ...
            $indexPage .= '<li><a href="'.$page.'.xhtml">'.$value['title']."</a></li>";
        }
        $indexPage = "$indexPage</ul></body></html>";
        ...
        $this->CreateFile( $this->temp_folder . '/OEBPS/page0.xhtml', $indexPage );
        // Create the zip file
        $this->CreateZip();
    }
    ...
    private function CreateZip () 
    {
        ...
        $cmd = "cd \"$this->temp_folder\"; zip -r \"$this->epub_file\" ./*; cd ; ";
    ...
    function download()
    {
        $filename = $this->temp_folder.$this->epub_file;
        ...
        ob_clean(); // Clear any previously written headers in the output buffer
                if(ini_get('zlib.output_compression'))
                    ...
                    header('Content-Disposition: attachment; filename="'.$this->epub_file.'"');
    ...
    function delete()
    {
        exec( "rm -r " .$this->temp_folder,$out);
